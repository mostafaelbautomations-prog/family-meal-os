...
- **Live (instant) mode is now the primary AI path** (user feedback: copy/paste felt like the product). Anywhere AI would fall back to copy/paste, an inline "Make it instant" card takes the API key, verifies it with a test call, saves it (localStorage) and flips `aiMode` to `live` — chats become instant mid-conversation. Saving a key in Settings also auto-switches to live. Manual copy/paste remains as the labelled "free fallback" (hard rule #3 parity intact), no longer presented as an equal-weight mode.
- **Recipes tab** (6th tab, `/recipes`): all recipes searchable, active sorted by name, retired collapsed. Shows compact macros, version, family average self-rating, and the self-improvement surface below.
- **Review-driven suggestions** (Dexie v4 `suggestions` table, additive; exported in backups as an optional table): an AI analysis pass reads ALL reviews of one recipe (cook-logged feedback + member self-ratings) and returns concrete recipe patches with exactly who supports each. Deterministic consensus rules (`src/lib/suggestions.ts`, unit-tested) then decide: **unanimous across active members (≥2) → auto-applied** as a version bump with "(whole family agreed)" in the changelog; **≥2 supporters → pending chip** ("Add more spice — Mom & Marwan think so") with Agree/No buttons in the Recipes tab and on the recipe screen; **single voice → dropped** (the weekly engine still sees every raw review). Per the user's rule, auto requires the *whole* family, not just everyone-who-reviewed.
- **Analysis runs are stateless recomputes**: each run passes open suggestions (to re-count supporters as reviews arrive) and applied/dismissed summaries ("never re-suggest"), then replaces the pending set. Dismissing is permanent by design. "New reviews since last analysis" is tracked as a review count in localStorage (disposable — worst case the button reappears).
- **Analysis triggers**: automatically right after a rating reply is saved (`/rate/return`, Live mode only — the moment "all members agreed" can become true), with the outcome shown before navigating; manually via "Check N new reviews" on the recipe screen (instant in Live, copy/paste card in Manual — parity kept). Not auto-run on cook feedback save: the cook is already in the app and one tap from the Recipes tab.
- **Per-person kitchen notes** (`PersonProfile.notes`, optional field — no migration needed): standing rules like "plate Marwan's portion before mixing in the vegetables". Three write paths: the cook edits them in Settings (one per line); review analysis extracts recurring per-person insights; both chats return a `personNotes` field when the cook states a lasting fact mid-conversation ("Remembered about Marwan: …"). Notes are injected into ALL prompts (weekly engine, recipe tweak, chef, review analysis) with instructions to bake them into prepSteps where relevant. They are cook-owned: the weekly engine's `profileUpdates` explicitly must not touch them, and `acceptDraft` preserves them when replacing likes/dislikes/patterns. Deduped case-insensitively on append.
- **Recipe-tweak chat now sees the family** (profiles + notes), so "make it spicier" won't violate a person's standing rules, and facts stated in chat are learned once and respected everywhere.
- **Instant meal quick-switch** (user asked for "a drop down menu next to it that can instantly change the meal"): a native `<select>` (`MealQuickSwitch`) on each planned meal that swaps the recipe the moment you pick one — no modal. Native select is deliberate — it opens the OS picker on phones (kitchen-friendly) and is instant. Options grouped via `<optgroup>` Family favorites / Recently cooked / All, with the current recipe pinned as the top "(now)" option. The favorites/recent/all grouping (previously inlined in the Today swap sheet) is extracted to `src/lib/recipeGroups.ts` (unit-tested) and shared. Placement: Today shows the dropdown inline with a small "More" link to the richer sheet (AI chef); Week shows it under each planned meal's name+status row. On Week the old swap-icon → `SwapSheet` modal is removed — the dropdown supersedes it (same groups, instant, no extra tap). Groups are computed once per screen and passed in (Week) or self-fetched (Today's ≤2 cards) to avoid N live queries. Only shown for `planned` meals; like every other swap it does not auto-regenerate groceries.
